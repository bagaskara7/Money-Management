...
/**
 * Created by bagaskara on 7/23/2015.
 */
public class SpinnerCategoriesAdapter extends ArrayAdapter<ItemData> {
    int mGroupId;
    ArrayList<ItemData> mList;
    LayoutInflater mInflater;

    public SpinnerCategoriesAdapter(Context context, int groupId, int id, ArrayList<ItemData> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (mList.get(position).getId() < 20) {
            int account = itemView.getContext().getResources().getIdentifier("category_" + mList.get(position).getText(), "string", itemView.getContext().getPackageName());
